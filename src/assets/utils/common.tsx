...
export function checkCesLicense() {
    axios.post(API.commons.checkCesLicense).then((res: any) => {
        if (res.body) {
 ...
 * 显示错误信息【登录】
 */
export function showErrorMsg(errorCode: any) {
    let err_msg;
...
export function _extends(target: any) { 
 ...
 * 图片加载错误时
 */
export function showErrorImg(isResc: Boolean) {
    if (isResc === true) {
...
export function replace_img(str: string) {
...
export function replace_htmlTag(str: string) {
    var strC = str.replace("&", "&amp;").replace("\"", "&quot;")
        .replace("<", "&lt;").replace(">", "&gt;").replace("'", "&#39;");
    return strC;
}

export function repalce_toHtml(str: string) {
    var strC = str.replace("&amp;", "&").replace("&quot;", "\"")
        .replace("&lt;", "<").replace("&gt;", ">").replace("&#39;", "'");
    return strC;
}
...
export function getDateDiff(dateStr: string) {
    ...
    var publishTime = getDateTimeStamp(dateStr) / 1000,
        ...
        Y = date.getFullYear(),
        M: ns = date.getMonth() + 1,
        D: ns = date.getDate(),
        H: ns = date.getHours(),
        m: ns = date.getMinutes(),
        s: ns = date.getSeconds();
    //小于10的在前面补0
    if (M < 10) {
        M = '0' + M;
    }
 ...
 * @returns {Element}
 */
export function createForm(url: string, params?: any) {
...
export var timeStamp_to_time = function (s: number) {
    var t;
    if (s > -1) {
...
export function strToDate(str: any) {
...
export function Base64() {
    ...
    var _utf8_encode = function (str: string) {
...
export function unique(arr?: any) {
    ...
    return Array.prototype.filter.call(arr, function (item: any, index: number) {
 ...
 * @returns {number}
 */
export function setCourseType(course?: any) {
    switch (course) {
        case 'COURSE_COMMON' :
            return 0;
            break;
